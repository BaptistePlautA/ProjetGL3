...
public class PointsControleArrondisHandler implements EventHandler<ActionEvent> {
    ...
    private final String numero = "0123456789";
    private int[] alphabetIndex = {0};
    private int[] numIndex = {0};
    char label = alphabet.charAt(alphabetIndex[0]);
    char label2 = numero.charAt(numIndex[0]);
    int i = 0;
    int n = 0;
    ...
    public PointsControleArrondisHandler (StackPane leftPane, StackPane rightPane){
    ...
    public void handleLeftPaneClick(MouseEvent event) {
        ...
        if ( n <= 1) {
        	label = alphabet.charAt(alphabetIndex[i]);
        	alphabetIndex[i]++;
        	n+=1;
        ...
        if(n > 1 && n<3){
        	n += 1;
        	System.out.println(n);
        	label2 = numero.charAt(numIndex[0]);
        	alphabetIndex[0]++;
        	numIndex[0]++;
        	pointsControleDebut.put(label2, pointDebut);
            pointsControleFin.put(label2, pointFin);
            
            afficher(pointDebut, pointFin, label2);
        }
        else {
			n = 0;
		}

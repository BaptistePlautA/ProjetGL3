...
public class PointsControleIntermediairesDeplacerHandler implements EventHandler<MouseEvent> {
    ...
    public void handle(MouseEvent event) {
        ...
        if (event.getEventType()==MouseEvent.MOUSE_DRAGGED) {
            ...
            double coordonneeX = cerclePointIntermediaire.getTranslateX() + deltaX; 
            double coordonneeY = cerclePointIntermediaire.getTranslateY() + deltaY; 
    
            //check cercle x pas en dehors du pane 
            if (coordonneeX < 0 - pane.getWidth()/2){
                cerclePointIntermediaire.setTranslateX(0 - pane.getWidth()/2); 
            }else if (coordonneeX > 0 + pane.getWidth()/2){
                cerclePointIntermediaire.setTranslateX(0 + pane.getWidth()/2);
                
            }else{
                cerclePointIntermediaire.setTranslateX(coordonneeX);
                indiceText.setTranslateX(indiceText.getTranslateX() + deltaX);
            }
            //check cercle y pas en dehors du pane
            if (coordonneeY < 0 - pane.getHeight()/2){
                cerclePointIntermediaire.setTranslateY(0 - pane.getHeight()/2); 
            }else if (coordonneeY > 0 + pane.getHeight()/2){
                cerclePointIntermediaire.setTranslateY(0 + pane.getHeight()/2);
            }else{
                cerclePointIntermediaire.setTranslateY(coordonneeY);
